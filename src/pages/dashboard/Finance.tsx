...
import { Wallet, TrendingUp, TrendingDown, Clock, AlertCircle, CheckCircle, Filter } from 'lucide-react';
import { mockPayments, mockClients, Payment } from '@/data/mockData';
...
const Finance = () => {
  ...
  const filteredPayments = payments.filter((payment) => {
    ...
    const matchesClient = filterClient === 'all' || payment.clientId === filterClient;
  ...
  const getTypeBadge = (type: string) => {
    ...
    return (
      <Badge variant="secondary" className="bg-pink-soft text-pink-dark border-0">
  ...
  const clientBudgets = mockClients.map((client) => {
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <StatCard
          ...
          icon={TrendingUp}
          change={`${Math.round((totalReceived / totalAll) * 100)}% dari total`}
          changeType="positive"
        />
        <StatCard
